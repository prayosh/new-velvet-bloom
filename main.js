...
function initGalleryLightbox() {
  ...
  function openLightbox(card) {
    if (!card) return;
    const mediaElement = (card.querySelector('img') || card.querySelector('svg')).cloneNode(true);
    
    // Clean animations and classes from showing inside preview
    mediaElement.removeAttribute('class');
    if (mediaElement.tagName.toLowerCase() === 'img') {
      mediaElement.style.maxWidth = '100%';
      mediaElement.style.maxHeight = '75vh';
      mediaElement.style.objectFit = 'contain';
      mediaElement.style.borderRadius = '12px';
      mediaElement.style.boxShadow = 'var(--shadow-glow)';
    }
  ...
  function navigateLightbox(direction) {
    ...
    setTimeout(() => {
      const mediaElement = (nextCard.querySelector('img') || nextCard.querySelector('svg')).cloneNode(true);
      mediaElement.removeAttribute('class');
      if (mediaElement.tagName.toLowerCase() === 'img') {
        mediaElement.style.maxWidth = '100%';
        mediaElement.style.maxHeight = '75vh';
